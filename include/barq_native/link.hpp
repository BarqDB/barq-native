...
#ifndef BARQ_NATIVE_BRIDGE_LINK_HPP
#define BARQ_NATIVE_BRIDGE_LINK_HPP
...
namespace barq::native {
    ...
    struct managed<T*> : managed_base {
        T* detach() const {
            T* v = new T();
            managed<T> m(m_obj->is_null(m_key) ? *m_obj : m_obj->get_linked_object(m_key), *m_barq);
        ...
        struct ref_type {
            ...
            bool operator ==(const managed<T*>& rhs) const {
                if (this->m_managed.m_barq != *rhs.m_barq) {
                    return false;
                }
            ...
            bool operator ==(const ref_type& rhs) const {
                if (this->m_managed.m_barq != rhs.m_managed.m_barq) {
                    return false;
                }
        ...
        ref_type operator ->() const {
            if (this->m_rbool_query) {
                ...
                return ref_type(managed<T>::prepare_for_query(*m_barq, this->m_rbool_query));
            }
            return ref_type(managed<T>(m_obj->get_linked_object(m_key), *m_barq));
        }
        operator bool() const {
            if (m_obj && m_key) {
        ...
        managed &operator=(T* o) {
            auto table = m_barq->table_for_object_type(managed<T>::schema.name);
            ...
            if (!o) {
                ...
                obj = table.create_object_with_primary_key(barq::native::internal::bridge::mixed(serialize(pk.value)));
            ...
            std::apply([&obj, &o, barq = *m_barq](auto && ...p) {
                (accessor<typename std::decay_t<decltype(p)>::Result>::set(
                         obj, obj.get_table().get_column_key(p.name), barq, (*o).*(std::decay_t<decltype(p)>::ptr)
                                 ), ...);
            }, managed<T>::schema.ps);
            return *this;
        }
...
#endif //BARQ_NATIVE_BRIDGE_LINK_HPP
