...
#ifndef BARQ_NATIVE_OBSERVATION_HPP
#define BARQ_NATIVE_OBSERVATION_HPP
...
namespace barq::native {
    ...
    struct object_change {
        ...
        const T *object;
        /// The object has been deleted from the Barq.
        ...
        /**
         ...
         only errors which can occur are when opening the Barq on a background
        ...
        void forward_change(const T *ptr,
            ...
            if (!ptr) {
                if (error) {
                    ...
                    oc.error = error;
                    block(std::forward<barq::native::object_change<T>>(std::move(oc)));
                } else {
                    auto oc = object_change<T>();
                    oc.is_deleted = true;
                    block(std::forward<barq::native::object_change<T>>(std::move(oc)));
    ...
    struct collection_callback_wrapper : internal::bridge::collection_change_callback {
        ...
        void before(const barq::native::internal::bridge::collection_change_set &) final {}
...
template <typename T>
inline std::ostream& operator<< (std::ostream& stream, const barq::native::object_change<T>& object)
{
    stream << "{\n";
...
#endif //BARQ_NATIVE_OBSERVATION_HPP
