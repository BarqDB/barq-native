...
#ifndef BARQ_NATIVE_RESULTS_HPP
#define BARQ_NATIVE_RESULTS_HPP
...
namespace barq::native {
    ...
    struct results_common_base {
        ...
        Derived where(std::function<rbool(managed<T>&)>&& fn) {
            ...
            auto barq = m_parent.get_barq();
            auto schema = barq.schema().find(managed<T>::schema.name);
        ...
        struct results_callback_wrapper : internal::bridge::collection_change_callback {
            ...
            void before(const barq::native::internal::bridge::collection_change_set&) override {}
        ...
        bool is_frozen() {
            return m_parent.get_barq().is_frozen();
        }
    ...
    struct results_base<T, Derived, results_is_mixed<T>> : public results_common_base<T, Derived> {
        ...
        // TODO: The current impl of barq::native::mixed does not allow managed object types,
        ...
        // properties on an object this use case is broken. Ideally we should replace barq::native::mixed to
        // not be a std::variant, but rather be a type-safe union we define in the SDK so that
        // barq::native::mixed could have a managed context itself.
        class iterator {
        public:
            using difference_type = size_t;
            using value_type = T;
    ...
    struct results_base<T, Derived, std::enable_if_t<managed<T>::is_object>> : public results_common_base<T, Derived> {
        ...
        public:
            ...
            value_type operator*() noexcept {
                ...
                return managed<T, void>(std::move(obj), this->m_parent->m_parent.get_barq());
            }
    ...
    struct linking_objects {
        ...
        static_assert(sizeof(managed<typename internal::ptr_type_extractor<ptr>::class_type>), "Type is not managed by the Barq");
    };
    ...
    private:
        results<Class> get_results() {
            ...
            auto schema = m_barq->schema().find(managed<Class>::schema.name);
            ...
            internal::bridge::results results(*m_barq, obj->get_backlink_view(m_barq->get_table(schema.table_key()), linking_property.column_key()));
            return ::barq::native::results<Class>(std::move(results));
        }
    };
}


#endif //BARQ_NATIVE_RESULTS_HPP
