...
#ifndef BARQ_NATIVE_MANAGED_MIXED_HPP
#define BARQ_NATIVE_MANAGED_MIXED_HPP
...
namespace barq::native {
    template <typename T>
    struct managed<T, std::enable_if_t<barq::native::internal::type_info::MixedPersistableConcept<T>::value>> : public managed_base {
        ...
        [[nodiscard]] stored_type get_stored_type() const {
            auto val = m_obj->get<barq::native::internal::bridge::mixed>(m_key);
            if (val.is_null()) {
        ...
        [[nodiscard]] T detach() const {
            return deserialize<T>(m_obj->get<barq::native::internal::bridge::mixed>(m_key));
        }
        ...
        typename managed<U>::ref_type get_stored_link() const {
            m_barq->read_group();
            barq::native::internal::bridge::mixed m = m_obj->get<barq::native::internal::bridge::mixed>(m_key);

            auto obj = internal::bridge::object(*m_barq, m.operator internal::bridge::obj_link());
            uint32_t alternative_key = m_barq->table_for_object_type(managed<std::remove_pointer_t<U>>::schema.name).get_key();
            ...
            return typename managed<U>::ref_type(managed<std::remove_pointer_t<U>>(obj.get_obj(), *m_barq));
        }
        ...
        void set_link(U &&v) {
            ...
            auto table = m_barq->table_for_object_type(managed<U>::schema.name);
            internal::bridge::obj o;
            if constexpr (managed<U>::schema.HasPrimaryKeyProperty) {
                auto pk = v.*(managed<U>::schema.primary_key().ptr);
                o = table.create_object_with_primary_key(barq::native::internal::bridge::mixed(serialize(pk.value)));
            } else {
            ...
            std::apply([&o, &v, this](auto && ...p) {
                (accessor<typename std::decay_t<decltype(p)>::Result>::set(
                         o, o.get_table().get_column_key(p.name), *this->m_barq, v.*(std::decay_t<decltype(p)>::ptr)
                                 ), ...);
            }, managed<U>::schema.ps);
    ...
    bool holds_alternative(const barq::native::managed<Types>& v) noexcept {
        auto val = v.get_stored_type();
        switch (val) {
            case barq::native::managed<Types>::stored_type::Int:
                if constexpr (std::is_same_v<T, int64_t>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::Bool:
                if constexpr (std::is_same_v<T, bool>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::String:
                if constexpr (std::is_same_v<T, std::string>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::Binary:
                if constexpr (std::is_same_v<T, std::vector<uint8_t>>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::Mixed:
                if constexpr (std::is_same_v<T, Types>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::Timestamp:
                if constexpr (std::is_same_v<T, std::chrono::time_point<std::chrono::system_clock>>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::Float:
                if constexpr (std::is_same_v<T, float>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::Double:
                if constexpr (std::is_same_v<T, double>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::Decimal:
                if constexpr (std::is_same_v<T, barq::native::decimal128>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::ObjectId:
                if constexpr (std::is_same_v<T, barq::native::object_id>)
                    return true;
                break;

            case barq::native::managed<Types>::stored_type::UUID:
                if constexpr (std::is_same_v<T, barq::native::uuid>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::Null:
                if constexpr (std::is_same_v<T, std::monostate>)
                    return true;
                break;
            case barq::native::managed<Types>::stored_type::TypedLink: {
                if constexpr (std::is_pointer_v<T>) {
                    auto m = v.m_obj->template get<internal::bridge::mixed>(v.m_key);
                    uint32_t alternative_key = v.m_barq->table_for_object_type(managed<std::remove_pointer_t<T>>::schema.name).get_key();
                    uint32_t stored_key = internal::bridge::object(*v.m_barq, m.operator internal::bridge::obj_link()).get_object_schema().table_key();
                    return alternative_key == stored_key;
                }
                break;
            }
            case barq::native::managed<Types>::stored_type::Link:
            case barq::native::managed<Types>::stored_type::LinkList:
                break;
            default:
                break;
        }
...
#endif//BARQ_NATIVE_MANAGED_MIXED_HPP
