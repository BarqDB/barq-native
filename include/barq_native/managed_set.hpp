...
#ifndef BARQ_NATIVE_MANAGED_SET_HPP
#define BARQ_NATIVE_MANAGED_SET_HPP
...
namespace barq::native {
    ...
    struct managed<std::set<T>, std::enable_if_t<internal::type_info::is_primitive<T>::value>> : managed_base {
        ...
        public:
            ...
            {
                auto s = barq::native::internal::bridge::set(*m_parent->m_barq, *m_parent->m_obj, m_parent->m_key);
        ...
        [[nodiscard]] std::set<T> detach() const {
            auto set = barq::native::internal::bridge::set(*m_barq, *m_obj, m_key);
            auto ret = std::set<T>();
        ...
        barq::native::notification_token observe(std::function<void(barq::native::collection_change)>&& fn) {
            auto set = std::make_shared<barq::native::internal::bridge::set>(*m_barq, *m_obj, m_key);
            barq::native::notification_token token = set->add_notification_callback(
        ...
        {
            auto set = internal::bridge::set(*m_barq, *m_obj, m_key);
        ...
        iterator find(const T& v)
        {
            ...
            if (idx == barq::native::not_in_collection)
                return iterator(size(), this);
            return iterator(idx, this);
        }
        void clear() {
            internal::bridge::set(*m_barq, *m_obj, m_key).remove_all();
        }

        size_t size()
        {
            return internal::bridge::set(*m_barq, *m_obj, m_key).size();
        }
    ...
    struct managed<std::set<T*>> : managed_base {
        ...
        public:
            ...
            {
                auto s = barq::native::internal::bridge::set(*m_parent->m_barq, *m_parent->m_obj, m_parent->m_key);
                managed<T> m(s.get_obj(m_i), *m_parent->m_barq);
                return {std::move(m)};
            }
        ...
        [[nodiscard]] std::set<T*> detach() const {
            auto s = barq::native::internal::bridge::set(*m_barq, *m_obj, m_key);
            size_t count = s.size();
            if (count == 0)
                return std::set<T*>();
            auto ret = std::set<T*>();
            for(size_t i = 0; i < count; i++) {
                managed<T> m(s.get_obj(i), *m_barq);
                T* v = new T();
...
#endif//BARQ_NATIVE_MANAGED_SET_HPP
