...
#ifndef BARQ_NATIVE_ACCESSORS_HPP
#define BARQ_NATIVE_ACCESSORS_HPP
...
namespace barq::native {
    ...
    struct accessor<std::vector<T*>> {
        static inline void set(internal::bridge::obj& obj,
            ...
            for (size_t i = 0; i < value.size(); i++) {
                ...
                std::apply([&m_obj, &lnk, &barq](auto && ...p) {
    ...
    struct accessor<std::set<T>> {
        static inline void set(internal::bridge::obj& obj,
            ...
            auto set = barq::native::internal::bridge::set(barq, obj, key);
        ...
        static void set(internal::bridge::obj& obj,
            ...
            for (auto& [k, v] : value) {
                if (v) {
                    ...
                    std::apply([&m_obj, &barq, o = *v](auto && ...p) {
                        (accessor<typename std::decay_t<decltype(p)>::Result>::set(
                                 m_obj, m_obj.get_table().get_column_key(p.name), barq,
    ...
    struct accessor<T*> {
        static inline void set(internal::bridge::obj& obj,
            ...
            std::apply([&m_obj, &barq, &value](auto && ...p) {
                (accessor<typename std::decay_t<decltype(p)>::Result>::set(
                         m_obj, m_obj.get_table().get_column_key(p.name), barq,
