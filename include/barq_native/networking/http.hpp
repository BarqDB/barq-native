...
#ifndef BARQ_NATIVE_NETWORKING_HTTP_HPP
#define BARQ_NATIVE_NETWORKING_HTTP_HPP

#include <barq_native/internal/bridge/barq.hpp>

#ifndef BARQ_NATIVE_VERSION_MAJOR
...
namespace barq::native::networking {
    ...
    struct default_http_transport : public http_transport_client {
        struct configuration {
            /**
             * Extra HTTP headers to be set on each request to Barq Sync when using the internal HTTP client.
             */
            std::optional<std::map<std::string, std::string>> custom_http_headers;
            /**
             * Network proxy configuration to be set on each request.
             */
            std::optional<::barq::native::internal::bridge::barq::sync_config::proxy_config> proxy_config;

            using SSLVerifyCallback = bool(const std::string& server_address,
                                           internal::bridge::barq::sync_config::proxy_config::port_type server_port,
        ...
        void send_request_to_server(const ::barq::native::networking::request& request,
                                    std::function<void(const ::barq::native::networking::response&)>&& completion);
...
#endif //BARQ_NATIVE_NETWORKING_HTTP_HPP
