...
#ifndef BARQ_NATIVE_MANAGED_DICTIONARY_HPP
#define BARQ_NATIVE_MANAGED_DICTIONARY_HPP
...
namespace barq::native {
    ...
    struct box_base {
        box_base(internal::bridge::core_dictionary &&backing_map,
                 ...
                 const internal::bridge::barq &r)
            : m_backing_map(std::move(backing_map)), m_key(key), m_barq(r) {}
        ...
        rbool operator==(const mapped_type &rhs) const {
            if constexpr (barq::native::internal::type_info::MixedPersistableConcept<mapped_type>::value) {
                if (this->m_rbool_query) {
                    return this->m_rbool_query->dictionary_has_value_for_key_equals(this->m_col_key, m_key, serialize(rhs, m_barq));
                }
                return m_backing_map.get(m_key) == serialize(rhs, m_barq);
            } else {
                if (this->m_rbool_query) {
        ...
        rbool operator!=(const mapped_type &rhs) const {
            if (this->m_rbool_query) {
                return this->m_rbool_query->dictionary_has_value_for_key_not_equals(this->m_col_key, m_key, internal::bridge::mixed(serialize(rhs, m_barq)));
            } else {
                return !operator==(rhs);
            }
        }
    ...
    struct box<uuid> : public box_base<uuid> {
        ...
        uuid operator*() {
            return this->m_backing_map.get(this->m_key).operator internal::bridge::uuid().operator ::barq::native::uuid();
        }
    };
    ...
    struct box<Mixed, std::enable_if_t<internal::type_info::MixedPersistableConcept<Mixed>::value>> : public box_base<Mixed> {
        ...
        rbool operator>(Mixed rhs) const {
            if (this->m_rbool_query) {
                return this->m_rbool_query->dictionary_has_value_for_key_greater_than(this->m_col_key, this->m_key, serialize(rhs, this->m_barq));
            }
            return this->m_backing_map.get(this->m_key) > serialize(rhs, this->m_barq);
        }

        rbool operator>=(Mixed rhs) const {
            if (this->m_rbool_query) {
                return this->m_rbool_query->dictionary_has_value_for_key_greater_than_equals(this->m_col_key, this->m_key, serialize(rhs, this->m_barq));
            }
            return this->m_backing_map.get(this->m_key) >= serialize(rhs, this->m_barq);
        }
        ...
        rbool operator<(Mixed rhs) const {
            if (this->m_rbool_query) {
                return this->m_rbool_query->dictionary_has_value_for_key_less_than(this->m_col_key, this->m_key, serialize(rhs, this->m_barq));
            }
            return this->m_backing_map.get(this->m_key) < serialize(rhs, this->m_barq);
        }
        ...
        rbool operator<=(Mixed rhs) const {
            if (this->m_rbool_query) {
                return this->m_rbool_query->dictionary_has_value_for_key_less_than_equals(this->m_col_key, this->m_key, serialize(rhs, this->m_barq));
            }
            return this->m_backing_map.get(this->m_key) <= serialize(rhs, this->m_barq);
        }
    };
    ...
    struct box<object_id> : public box_base<object_id> {
        ...
        object_id operator*() {
            return this->m_backing_map.get(this->m_key).operator internal::bridge::object_id().operator ::barq::native::object_id();
        }
    };
    template<>
    struct box<decimal128> : public box_base<decimal128> {
        ...
        decimal128 operator*() {
            return this->m_backing_map.get(this->m_key).operator internal::bridge::decimal128().operator ::barq::native::decimal128();
        }
    ...
    struct box<managed<V*>> : public box_base<managed<V*>> {
        ...
        std::optional<typename managed<V*>::ref_type> operator*() {
            ...
            return typename managed<V*>::ref_type(managed<V>(std::move(obj), this->m_barq));
        }
        ...
        typename managed<V*>::ref_type operator->() {
            if (this->m_rbool_query) {
                ...
                return typename managed<V*>::ref_type(managed<V>::prepare_for_query(this->m_barq, this->m_rbool_query));
            }
            auto obj = this->m_backing_map.get_object(this->m_key);
            return typename managed<V*>::ref_type(managed<V>(std::move(obj), this->m_barq));
        }
        ...
        box& operator=(V* o) {
            if (!o) {
                ...
                this->m_backing_map.insert(this->m_key, barq::native::internal::bridge::mixed());
                return *this;
            }
            internal::bridge::obj m_obj;
            ...
            std::apply([&m_obj, &o, barq = this->m_barq](auto && ...p) {
                (accessor<typename std::decay_t<decltype(p)>::Result>::set(
    ...
    struct managed<std::map<std::string, T>, void> : managed_base {
        ...
        [[nodiscard]] std::map<std::string, T> detach() const {
            if constexpr (std::is_pointer_v<T>) {
                ...
                for (size_t i = 0; i < s; i++) {
                    ...
                    T v = new Type;
                    managed<Type, void> m(d.get_object(pair.first), *m_barq);
        ...
        box<std::conditional_t<std::is_pointer_v<T>, managed<T>, T>>  operator[](const std::string &key) {
            if constexpr (std::is_pointer_v<T>) {
                if (m_rbool_query) {
                    return box<managed<T>>(m_rbool_query, m_key, key, *m_barq);
                }
                return box<managed<T>>(m_obj->get_dictionary(m_key), key, *m_barq);
            } else {
                if (m_rbool_query) {
                    return box<T>(m_rbool_query, m_key, key, *m_barq);
                }
                return box<T>(m_obj->get_dictionary(m_key), key, *m_barq);
            }
        }
        ...
        notification_token observe(std::function<void(barq::native::dictionary_collection_change)>&& fn)
        {
            auto o = internal::bridge::object(*m_barq, *m_obj);
            auto dict = std::make_shared<barq::native::internal::bridge::dictionary>(o.get_dictionary(m_key));
            barq::native::notification_token token = dict->add_key_based_notification_callback(
                                                std::make_shared<barq::native::dictionary_callback_wrapper>(std::move(fn), false));
            token.m_barq = *m_barq;
...
#endif//BARQ_NATIVE_MANAGED_DICTIONARY_HPP
