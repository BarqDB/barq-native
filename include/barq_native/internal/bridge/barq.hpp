...
#ifndef BARQ_NATIVE_BRIDGE_BARQ_HPP
#define BARQ_NATIVE_BRIDGE_BARQ_HPP
...
namespace barq::native {
    ...
    enum class client_reset_mode: uint8_t {
        ...
        manual,
        // Discard unsynced local changes, without disrupting accessors or closing the Barq
        discard_unsynced,
...
namespace barq::native::internal::bridge {
    ...
    struct barq {
        enum class sync_session_stop_policy: uint8_t {
            immediately,          // Immediately stop the session as soon as all Barq instances/Sessions go out of scope.
            live_indefinitely,     // Never stop the session.
            after_changes_uploaded, // Once all Barq instances/Sessions go out of scope, wait for uploads to complete and stop.
        };
        ...
        struct config {
            ...
            enum class schema_mode : uint8_t {
                ...
                // function, to support sharing a Barq file between processes using
                ...
                // Open the Barq in read-only mode, transactions are not allowed to
                // be performed on the Barq instance. The schema of the existing Barq
                // file won't be changed through this Barq instance. Extra tables and
                // extra properties are allowed in the existing Barq schema. The
                ...
                // mode, sync Barq can be opened with ReadOnly mode. Changes
                // can be made to the Barq file through another writable Barq instance.
                ...
                // if the Barq is already open on another thread.
                additive_discovered,
            ...
            ~config();
            config(const BarqConfig&); //NOLINT(google-explicit-constructor)
            ...
            operator BarqConfig() const; //NOLINT(google-explicit-constructor)
            ...
            /// Open the Barq using the sync history mode even if a sync configuration is not supplied.
            ...
            void set_client_reset_handler(const client_reset_mode_base<T>& handler) {
                before_client_reset([fn = std::move(handler.m_before)](barq local_barq) {
        ...
        private:
            ...
            void before_client_reset(std::function<void(barq old_barq)> callback);
            void after_client_reset(std::function<void(barq local_barq, barq remote_barq)> callback);
            inline BarqConfig* get_config();
            inline const BarqConfig* get_config() const;
#ifdef BARQ_NATIVE_HAVE_GENERATED_BRIDGE_TYPES
            storage::Barq_Config m_config[1];
#else
            std::shared_ptr<BarqConfig> m_config;
#endif
        };

        barq();
        barq(const config&); //NOLINT(google-explicit-constructor)
        barq(std::shared_ptr<Barq>); //NOLINT(google-explicit-constructor)
        barq(thread_safe_reference&& tsr, const std::optional<std::shared_ptr<scheduler>>&); //NOLINT(google-explicit-constructor)
        operator std::shared_ptr<Barq>() const; //NOLINT(google-explicit-constructor)
        group read_group();
        ...
        [[nodiscard]] std::shared_ptr<struct scheduler> scheduler() const;
        static async_open_task get_synchronized_barq(const config&);
        bool refresh();
    ...
    protected:
        ...
        client_reset_mode m_mode;
        friend struct internal::bridge::barq::config;
    };

    template <typename T>
    T resolve(const barq&, thread_safe_reference&& tsr);
    template <>
    dictionary resolve(const barq&, thread_safe_reference&& tsr);
    template <>
    object resolve(const barq&, thread_safe_reference&& tsr);

    bool operator ==(const barq&, const barq&);
    bool operator !=(const barq&, const barq&);
}

#endif //BARQ_NATIVE_BRIDGE_BARQ_HPP
