...
#ifndef BARQ_NATIVE_MACROS_HPP
#define BARQ_NATIVE_MACROS_HPP
...
#define DECLARE_PROPERTY(cls, p) barq::native::property<&cls::p>(#p),
#define DECLARE_MANAGED_PROPERTY(cls, p) &barq::native::managed<cls>::p,
...
namespace barq::native {
    ...
    public:
        ...
        void prepare_for_query(internal::bridge::barq* barq,
                               const internal::bridge::table& table,
                               const std::string_view& col_name,
                               barq::native::rbool* query_builder) {
            this->m_barq = barq;
...
#define __barq_native_build_query(op, name, type) \
...
#define __barq_native_build_optional_query(op, name, type) \
...
#define BARQ_SCHEMA(cls, ...) \
    DECLARE_BARQ_SCHEMA(cls, false, false, ObjectType::TopLevel, __VA_ARGS__) \

#define BARQ_EMBEDDED_SCHEMA(cls, ...) \
    DECLARE_BARQ_SCHEMA(cls, true, false, ObjectType::Embedded, __VA_ARGS__)

#define BARQ_ASYMMETRIC_SCHEMA(cls, ...) \
    DECLARE_BARQ_SCHEMA(cls, false, true, ObjectType::Asymmetric, __VA_ARGS__)

#define DECLARE_BARQ_SCHEMA(cls, is_embedded_object, is_asymmetric_object, obj_type, ...) \
    template <> struct managed<cls> { \
        managed() = default; \
        static constexpr ObjectType object_type = obj_type;     \
        FOR_EACH(DECLARE_PERSISTED, cls, __VA_ARGS__) \
        static constexpr auto schema = barq::native::schema(#cls, object_type, std::tuple{ FOR_EACH(DECLARE_PROPERTY, cls, __VA_ARGS__) }  ); \
        ...
        explicit managed(const internal::bridge::obj& obj,                                                      \
                         internal::bridge::barq barq)                                                         \
        : m_obj(std::move(obj))                                                                                 \
        , m_barq(std::move(barq))                                                                             \
        {                                                                                                       \
            auto zipped = internal::zip_tuples(managed_pointers(), managed_pointers_names());                   \
            std::apply([&](auto& ...pair) {                                                                     \
                ((*this.*pair.first).assign(&m_obj, &m_barq, m_obj.get_table().get_column_key(pair.second)), ...); \
            }, zipped);                                                                                         \
        }                                                                                                       \
        managed(const managed& other) {                                                                         \
            ...
            auto zipped = internal::zip_tuples(managed_pointers(), managed_pointers_names());                   \
            if (m_rbool_query) {                                                                                \
                auto schema = m_barq.schema().find(other.schema.name);                                         \
                auto group = m_barq.read_group();                                                              \
                auto table_ref = group.get_table(schema.table_key());                                           \
                std::apply([&](auto& ...pair) {                                                                 \
                    ((*this.*pair.first).prepare_for_query(&m_barq, table_ref, pair.second, m_rbool_query), ...);  \
                }, zipped);                                                                                     \
            } else {                                                                                            \
                std::apply([&](auto& ...pair) {                                                                 \
                    ((*this.*pair.first).assign(&m_obj, &m_barq, m_obj.get_table().get_column_key(pair.second)), ...); \
        ...
        static managed prepare_for_query(const internal::bridge::barq& r, barq::native::rbool* q) {                   \
            managed<cls> m;                                                                                     \
            m.m_barq = r;                                                                                      \
            m.m_rbool_query = q;                                                                                \
            auto schema = m.m_barq.schema().find(m.schema.name);                                               \
            auto group = m.m_barq.read_group();                                                                \
            auto table_ref = group.get_table(schema.table_key());                                               \
            auto zipped = internal::zip_tuples(managed_pointers(), managed_pointers_names());                   \
            std::apply([&m, &table_ref](auto& ...pair) {                                                        \
                ((m.*pair.first).prepare_for_query(&m.m_barq, table_ref, pair.second, m.m_rbool_query), ...);  \
            }, zipped);                                                                             \
            return m;                                                                               \
        }                                                                                           \
        cls detach() const {                                                                        \
        ...
        auto observe(std::function<void(barq::native::object_change<managed>&&)>&& fn) {                   \
            auto m_object = std::make_shared<internal::bridge::object>(m_barq, m_obj);             \
            auto wrapper = barq::native::object_change_callback_wrapper<managed>{                          \
            std::move(fn), this, m_object};                                                         \
            return m_object->add_notification_callback(                                             \
            std::make_shared<barq::native::object_change_callback_wrapper<managed>>(wrapper));             \
        }                                                                                           \
        bool is_invalidated() {                                                                     \
        ...
        managed<cls> freeze() {                                                                     \
            auto barq = m_barq.freeze();                                                          \
            return managed<cls>(barq.import_copy_of(m_obj), barq);                                \
        }                                                                                           \
        managed<cls> thaw() {                                                                       \
            if (is_invalidated()) {                                                                 \
            ...
            auto thawed_barq = m_barq.thaw();                                                     \
            return managed<cls>(thawed_barq.import_copy_of(m_obj), thawed_barq);                  \
        }                                                                                           \
        db get_barq() {                                                                            \
            return db(m_barq);                                                                     \
        }                                                                                           \
    ...
    private:                                                                                        \
        ...
        template <typename> friend struct ::barq::native::thread_safe_reference;                           \
        template <typename T> friend rbool* ::barq::native::internal::get_rbool(const T&);                 \
                                                                                                    \
    };                                                                                              \
