...
namespace barq::native {
    std::vector<uint8_t> managed<std::vector<uint8_t>>::detach() const {
        return m_obj->template get<barq::native::internal::bridge::binary>(m_key);
    }
    ...
    __barq_native_build_query(==, equal, std::vector<uint8_t>)
    __barq_native_build_query(!=, not_equal, std::vector<uint8_t>)

    // MARK: Optional

    std::optional<std::vector<uint8_t>> managed<std::optional<std::vector<uint8_t>>>::detach() const {
        return m_obj->get_optional<barq::native::internal::bridge::binary>(m_key);
    }
    ...
    __barq_native_build_optional_query(==, equal, std::vector<uint8_t>)
    __barq_native_build_optional_query(!=, not_equal, std::vector<uint8_t>)
}
