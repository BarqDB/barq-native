...
namespace barq::native {
    ...
    rbool managed_string::contains(const std::string &rhs, bool case_sensitive) const noexcept {
        ...
        return detach().find(rhs) != barq::native::not_in_collection;
    }
    ...
    __barq_native_build_query(==, equal, std::string)
    __barq_native_build_query(!=, not_equal, std::string)

    __barq_native_build_optional_query(==, equal, std::string)
    __barq_native_build_optional_query(!=, not_equal, std::string)
}
