...
namespace barq::native::internal::bridge {
    ...
    list::list(const barq &barq,
               const obj &obj,
               const col_key& col_key) {
#ifdef BARQ_NATIVE_HAVE_GENERATED_BRIDGE_TYPES
        new (&m_list) List(object(barq, obj).get_list(col_key));
#else
        m_list = std::make_shared<List>(List(object(barq, obj).get_list(col_key)));
#endif
    }
    ...
    void list::add(const mixed &v) {
        get_list()->add(v.operator ::barq::Mixed());
    }
    ...
    void list::set(size_t pos, const mixed &v) { get_list()->set(pos, v.operator ::barq::Mixed()); }
    ...
    size_t list::find(const mixed &v) { return get_list()->find(v.operator ::barq::Mixed()); }
