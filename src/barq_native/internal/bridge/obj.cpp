...
namespace barq::native::internal::bridge {
    ...
    group::group(barq& val)
    : m_barq(val)
    {
    }

    group barq::read_group() {
        return *this;
    }
      ...
      if (value.is_null()) {
          get_obj()->set(col_key, Mixed());
           ...
           get_obj()->set(col_key, Mixed(ObjLink(get_obj()->get_table()->get_key(),
            ...
            get_obj()->set(col_key, value.operator ::barq::Mixed());
        }
    }
    ...
    table group::get_table(const std::string &table_key) {
        return static_cast<SharedBarq>(m_barq.get())->read_group().get_table(table_name_for_object_type(table_key));
    }
    table group::get_table(uint32_t table_key) {
        return static_cast<SharedBarq>(m_barq.get())->read_group().get_table(TableKey(table_key));
    }
...
std::string barq::native::internal::bridge::table_name_for_object_type(const std::string &v) {
