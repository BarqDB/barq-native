...
namespace barq::native::internal::bridge {
    ...
    object::object(const barq &barq, const struct obj &obj) {
#ifdef BARQ_NATIVE_HAVE_GENERATED_BRIDGE_TYPES
        new (&m_object) Object(barq, obj);
#else
        m_object = std::make_shared<Object>(barq, obj);
#endif
    }
    object::object(const barq &barq, const struct obj_link& link) {
#ifdef BARQ_NATIVE_HAVE_GENERATED_BRIDGE_TYPES
        new (&m_object) Object(barq, link);
#else
        m_object = std::make_shared<Object>(barq, link);
#endif
    }
    ...
    list object::get_list(const col_key& col_key) const {
        return List(get_barq(), get_obj(), col_key);
    }
    dictionary object::get_dictionary(const col_key &v) const {
        return Dictionary(get_barq(), get_obj(), v);
    }
    ...
    barq object::get_barq() const {
#ifdef BARQ_NATIVE_HAVE_GENERATED_BRIDGE_TYPES
        return reinterpret_cast<const Object*>(&m_object)->get_barq();
#else
        return m_object->get_barq();
#endif
    }
