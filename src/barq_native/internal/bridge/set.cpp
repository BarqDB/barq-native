...
#include <barq/array_mixed.hpp>
#include <barq/array_typed_link.hpp>
...
namespace barq::native::internal::bridge {
    ...
    set::set(const barq &barq,
               const obj &obj,
               const col_key& col_key) {
#ifdef BARQ_NATIVE_HAVE_GENERATED_BRIDGE_TYPES
        new (&m_set) object_store::Set(barq.operator std::shared_ptr<Barq>(), obj, col_key);
#else
        m_set = std::make_shared<object_store::Set>(barq.operator std::shared_ptr<Barq>(), obj, col_key);
#endif
    }
    ...
    std::pair<size_t, bool> set::insert(const mixed &v) {
        return get_set()->insert(v.operator ::barq::Mixed());
    }
    ...
    void set::remove(const mixed &v) { get_set()->remove(v.operator ::barq::Mixed()); }
    ...
    size_t set::find(const mixed &v) { return get_set()->find(v.operator ::barq::Mixed()); }
