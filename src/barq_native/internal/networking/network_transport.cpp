...
#if __has_include(<barq/util/config.h>)
...
namespace barq::native::networking {
    ...
    struct DefaultSocket : barq::sync::network::Socket {
        DefaultSocket(barq::sync::network::Service& service)
            : barq::sync::network::Socket(service)
        {
        }
        ...
        void async_write(const char* data, std::size_t size, H handler)
        {
            if (ssl_stream) {
                ...
                barq::sync::network::Socket::async_write(data, size, std::move(handler)); // Throws
            }
        }
        ...
        void async_read_until(char* buffer, std::size_t size, char delim, H handler)
        {
            if (ssl_stream) {
                ...
                barq::sync::network::Socket::async_read_until(buffer, size, delim, m_read_buffer, std::move(handler));
            }
        }
        ...
        void async_read(char* buffer, std::size_t size, H handler)
        {
            if (ssl_stream) {
                ...
                barq::sync::network::Socket::async_read(buffer, size, m_read_buffer, std::move(handler));
            }
        }
        ...
        {
            if (!ssl_stream) {
                BARQ_TERMINATE("ssl_stream is not initialized");
            }
            ssl_stream->async_handshake(std::move(handler)); // Throws
        }

        util::Optional<barq::sync::network::ssl::Stream> ssl_stream;
    private:
        barq::sync::network::ReadAheadBuffer m_read_buffer;
    };
    ...
    void default_http_transport::send_request_to_server(const ::barq::native::networking::request& request,
                                                        std::function<void(const ::barq::native::networking::response&)>&& completion_block) {
        const auto uri = barq::util::Uri(request.url);
        std::string userinfo, host, port;
        uri.get_auth(userinfo, host, port);

        barq::sync::network::Service service;
        using namespace barq::sync::network::ssl;
        Context m_ssl_context;
        DefaultSocket socket{service};
        barq::sync::network::Endpoint ep;
        ...
        try {
            auto resolver = barq::sync::network::Resolver{service};
            if (m_configuration.proxy_config) {
                std::string proxy_userinfo, proxy_host, proxy_port;
                const auto proxy_uri = barq::util::Uri(m_configuration.proxy_config->address);
                proxy_uri.get_auth(proxy_userinfo, proxy_host, proxy_port);
                if (proxy_host.empty()) {
                    std::error_code e;
                    auto address = barq::sync::network::make_address(m_configuration.proxy_config->address, e);
                    if (e.value() > 0) {
                        ::barq::native::networking::response response;
                        response.custom_status_code = e.value();
                        response.body = e.message();
                        completion_block(std::move(response));
                        return;
                    }
                    ep = barq::sync::network::Endpoint(address, m_configuration.proxy_config->port);
                } else {
                ...
                if (is_ipv4) {
                    std::error_code e;
                    auto address = barq::sync::network::make_address(host, e);
                    ep = barq::sync::network::Endpoint(address, stoi(port));
                } else {
        ...
        if (m_configuration.proxy_config) {
            barq::sync::HTTPRequest req;
            req.method = barq::sync::HTTPMethod::Connect;
            ...
            barq::sync::HTTPClient<DefaultSocket> m_proxy_client = barq::sync::HTTPClient<DefaultSocket>(socket, logger);
            auto handler = [&](barq::sync::HTTPResponse response, std::error_code ec) {
                if (ec && ec != util::error::operation_aborted) {
                    ::barq::native::networking::response res;
                    res.custom_status_code = util::error::operation_aborted;
                    completion_block(std::move(res));
                    return;
                }

                if (response.status != barq::sync::HTTPStatus::Ok) {
                    ::barq::native::networking::response res;
        ...
        barq::sync::HTTPHeaders headers;
        for (auto& [k, v] : request.headers) {
            headers[k] = v;
        }
        headers["Host"] = host;
        headers["User-Agent"] = "Barq Native";
        ...
        barq::sync::HTTPClient<DefaultSocket> m_http_client = barq::sync::HTTPClient<DefaultSocket>(socket, logger);
        barq::sync::HTTPMethod method;
        switch (request.method) {
            case ::barq::native::networking::http_method::get:
                method = barq::sync::HTTPMethod::Get;
                break;
            case ::barq::native::networking::http_method::put:
                method = barq::sync::HTTPMethod::Put;
                break;
            case ::barq::native::networking::http_method::post:
                method = barq::sync::HTTPMethod::Post;
                break;
            case ::barq::native::networking::http_method::patch:
                method = barq::sync::HTTPMethod::Patch;
                break;
            case ::barq::native::networking::http_method::del:
                method = barq::sync::HTTPMethod::Delete;
                break;
            default:
                BARQ_UNREACHABLE();
        }
        ...
        service.post([&](barq::Status&&){
            auto handler = [&](std::error_code ec) {
                if (ec.value() == 0) {
                    ...
                    m_http_client.async_request(std::move(req), [cb = std::move(completion_block)](const barq::sync::HTTPResponse& r, const std::error_code&) {
                        ::barq::native::networking::response res;
