...
TEST_CASE("numerics", "[numerics]") {
    ...
    SECTION("unmanaged_managed_mixed_get_set", "[mixed]") {
        ...
        obj.double_col = 1.0;
        obj.bool_col = true;
        obj.enum_col = AllTypesObject::Enum::one;
        ...
        CHECK(managed_obj.enum_col == AllTypesObject::Enum::one);
        CHECK(managed_obj.enum_col != AllTypesObject::Enum::two);

        res = barq.objects<AllTypesObject>().where([](auto&& o) {
            return o.enum_col == AllTypesObject::Enum::one;
        });
        CHECK(res.size() == 1);
        res = barq.objects<AllTypesObject>().where([](auto&& o) {
            return o.enum_col != AllTypesObject::Enum::one;
        ...
        barq.write([&barq, &managed_obj] {
            managed_obj.enum_col = AllTypesObject::Enum::two;
            CHECK(managed_obj.enum_col == AllTypesObject::Enum::two);
        });

        // Opt Enum
        CHECK(managed_obj.opt_enum_col == AllTypesObject::Enum::one);
        CHECK(managed_obj.opt_enum_col != AllTypesObject::Enum::two);

        res = barq.objects<AllTypesObject>().where([](auto&& o) {
            return o.opt_enum_col == AllTypesObject::Enum::one;
        });
        CHECK(res.size() == 1);
        res = barq.objects<AllTypesObject>().where([](auto&& o) {
            return o.opt_enum_col != AllTypesObject::Enum::one;
        ...
        barq.write([&barq, &managed_obj] {
            managed_obj.opt_enum_col = AllTypesObject::Enum::two;
            CHECK(managed_obj.opt_enum_col == AllTypesObject::Enum::two);
        });
