...
TEST_CASE("date", "[date]") {
    ...
    SECTION("unmanaged_managed_ts_time_since_epoch", "[date]") {
        ...
        barq::native::AllTypesObject object;
        object.date_col = ts;
    ...
    SECTION("add") {
        auto barq = barq::native::db(std::move(config));
        barq::native::AllTypesObject obj;
    ...
    SECTION("rbool_comparison") {
        ...
        auto results = barq.objects<barq::native::AllTypesObject>().where([&now](auto& o) {
            return o.date_col == now;
        });
        CHECK(results.size() == 1);
        CHECK(results[0].date_col.time_since_epoch() == now.time_since_epoch());

        results = barq.objects<barq::native::AllTypesObject>().where([&now](auto& o) {
            return o.date_col != now;
        });
